...
Elm.Hypercube = Elm.Hypercube || {};
Elm.Hypercube.make = function (_elm) {
   "use strict";
   _elm.Hypercube = _elm.Hypercube || {};
   if (_elm.Hypercube.values) return _elm.Hypercube.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var requestDimension = Elm.Native.Port.make(_elm).inboundSignal("requestDimension",
   "( Float, Int )",
   function (v) {
   ...
   var Bundle = F3(function (a,b,c) {    return {vertices: a,edges: b,rotationMatrices: c};});
   var zeroMat = F2(function (m,n) {    return A2($List.repeat,m,A2($List.repeat,n,0));});
   var matDim = function (m) {    var _p0 = m;if (_p0.ctor === "::") {    return $List.length(_p0._0);} else {    return 0;}};
   var factorial = function (n) {    return _U.cmp(n,1) < 0 ? n : n * factorial(n - 1);};
   var emptyMatrix = _U.list([_U.list([])]);
   var idMat = function (n) {
      if (_U.cmp(n,0) < 1) return emptyMatrix; else {
            var same = F2(function (x,y) {    return _U.eq(x,y) ? 1 : 0;});
            var idRow = function (x) {    return A2($List.map,same(x),_U.range(0,n - 1));};
            return A2($List.map,idRow,_U.range(0,n - 1));
         }
   };
   var allOrNothing = function (maybeList) {
      var maybeAccumulate = F2(function (x,acc) {
         var _p1 = {ctor: "_Tuple2",_0: x,_1: acc};
         if (_p1._0.ctor === "Nothing") {
               ...
               if (_p1._1.ctor === "Nothing") {
                     ...
                     return $Maybe.Just(A2($List._op["::"],_p1._0._0,_p1._1._0));
                  }
            }
      });
      return A3($List.foldl,maybeAccumulate,$Maybe.Just(_U.list([])),maybeList);
   };
   var bin2vec = function (bin) {    var binList = A2($String.split,"",bin);return A2($List.map,function (x) {    return _U.eq(x,"0") ? 0 : 1;},binList);};
   var binStringAcc = F2(function (i,sofar) {
      binStringAcc: while (true) {
         var next = i / 2 | 0;
         var parity = A2($Basics._op["%"],i,2);
         var _p2 = i;
         switch (_p2)
         {case 0: return A2($List._op["::"],_U.chr("0"),sofar);
            case 1: return A2($List._op["::"],_U.chr("1"),sofar);
            default: var _p3 = parity;
              if (_p3 === 1) {
                    var _v4 = next,_v5 = A2($List._op["::"],_U.chr("1"),sofar);
                    i = _v4;
                    sofar = _v5;
                    continue binStringAcc;
                 } else {
                    var _v6 = next,_v7 = A2($List._op["::"],_U.chr("0"),sofar);
                    i = _v6;
                    sofar = _v7;
                    continue binStringAcc;
                 }}
   ...
   var binString = function (i) {    return $String.fromList(A2(binStringAcc,i,_U.list([])));};
   var paddedBinString = F2(function (len,i) {    return A3($String.padLeft,len,_U.chr("0"),binString(i));});
   var vertices = function (n) {    var numVerts = Math.pow(2,n);return A2($List.map,bin2vec,A2($List.map,paddedBinString(n),_U.range(0,numVerts - 1)));};
   var dupeString = F2(function (s,n) {    return _U.cmp(n,0) < 1 ? "" : A2($Basics._op["++"],s,A2(dupeString,s,n - 1));});
   var appendTuple = function (_p4) {    var _p5 = _p4;return A2($List.append,_p5._0,_p5._1);};
   var concatIndexedMap = F2(function (fn,list) {    return $List.concat(A2($List.indexedMap,fn,list));});
   var zip = F2(function (xs,ys) {
      var _p6 = {ctor: "_Tuple2",_0: xs,_1: ys};
      if (_p6._0.ctor === "::" && _p6._1.ctor === "::") {
            return A2($List._op["::"],{ctor: "_Tuple2",_0: _p6._0._0,_1: _p6._1._0},A2(zip,_p6._0._1,_p6._1._1));
         } else {
            return _U.list([]);
         }
   });
   var indexedFilterMap = F2(function (fn,list) {
      var fn$ = function (_p7) {    var _p8 = _p7;return A2(fn,_p8._0,_p8._1);};
      var len = $List.length(list);
      return A2($List.filterMap,fn$,A2(zip,_U.range(0,len - 1),list));
   });
   var verticesWithEdges = function (n) {
      var biggerNeighbours = F2(function (index,vertex) {
         var getEdgeMates = F2(function (twosPlaceInverted,digit) {
            var twosPlace = n - twosPlaceInverted;
            var _p9 = digit;
            if (_p9 === 0) {
                  return $Maybe.Just({ctor: "_Tuple2",_0: index,_1: index + Math.pow(2,twosPlace - 1)});
               } else {
                  return $Maybe.Nothing;
               }
         });
         return A2(indexedFilterMap,getEdgeMates,vertex);
      });
      var verts = vertices(n);
      var pairs = A2(concatIndexedMap,biggerNeighbours,verts);
      var numVerts = Math.pow(2,n);
      return {ctor: "_Tuple2",_0: verts,_1: pairs};
   };
   var appendBlockRow = function (blocks) {
      var _p10 = blocks;
      if (_p10.ctor === "::") {
            if (_p10._1.ctor === "[]") {
                  return _p10._0;
               } else {
                  var appendedRow = appendBlockRow(_p10._1);
                  return A2($List.map,appendTuple,A2(zip,_p10._0,appendedRow));
               }
         } else {
            return emptyMatrix;
         }
   };
   var appendBlocks = function (blockMat) {
      appendBlocks: while (true) {
         var _p11 = blockMat;
         if (_p11.ctor === "::") {
               if (_p11._1.ctor === "[]") {
                     return appendBlockRow(_p11._0);
                  } else {
                     var _p14 = _p11._1;
                     var _p13 = _p11._0;
                     var _p12 = _p13;
                     if (_p12.ctor === "[]") {
                           var _v15 = _p14;
                           blockMat = _v15;
                           continue appendBlocks;
                        } else {
                           if (_p12._0.ctor === "::" && _p12._0._0.ctor === "[]" && _p12._0._1.ctor === "[]") {
                                 var _v16 = _p14;
                                 blockMat = _v16;
                                 continue appendBlocks;
                              } else {
                                 var restRows = appendBlocks(_p14);
                                 var appendedBlocks = appendBlockRow(_p13);
                                 return A2($List.append,appendedBlocks,restRows);
                              }
                        }
                  }
            } else {
   ...
   var appendBlocks2x2 = F4(function (tl,tr,bl,br) {    return appendBlocks(_U.list([_U.list([tl,tr]),_U.list([bl,br])]));});
   var blockDiag = function (blocks) {
      blockDiag: while (true) {
         var _p15 = blocks;
         if (_p15.ctor === "::") {
               var _p19 = _p15._1;
               var _p18 = _p15._0;
               var _p16 = _p18;
               if (_p16.ctor === "::") {
                     var bottomRight = blockDiag(_p19);
                     var topLeft = _p18;
                     var n = $List.length(_p16._0);
                     var m = $List.length(_p18);
                     var _p17 = bottomRight;
                     if (_p17.ctor === "::") {
                           if (_p17._0.ctor === "[]" && _p17._1.ctor === "[]") {
                                 return topLeft;
                              } else {
                                 var n$ = $List.length(_p17._0);
                                 var topRight = A2(zeroMat,m,n$);
                                 var m$ = $List.length(_p17._1) + 1;
                                 var bottomLeft = A2(zeroMat,m$,n);
                                 return A4(appendBlocks2x2,topLeft,topRight,bottomLeft,bottomRight);
                              }
                        } else {
                           return emptyMatrix;
                        }
   ...
   var zeroPadMat = F3(function (m,n,subMat) {
      var tl = subMat;
      var m$ = m - 1;
      var bl = A2(zeroMat,m$,1);
      var n$ = n - 1;
      var tr = A2(zeroMat,1,n$);
      var br = A2(zeroMat,m$,n$);
      return A4(appendBlocks2x2,tl,tr,bl,br);
   });
   var rotMatR = F2(function (theta,n) {
      var rankI = n - 2;
      var zRow = A2(zeroMat,1,rankI);
      var zCol = A2(zeroMat,rankI,1);
      var i = idMat(rankI);
      var sinTheta = $Basics.sin(theta);
      var negSinTheta = $Basics.negate(sinTheta);
      var ns = _U.list([_U.list([negSinTheta])]);
      var s = _U.list([_U.list([sinTheta])]);
      var cosTheta = $Basics.cos(theta);
      var c = _U.list([_U.list([cosTheta])]);
      var blocks = _U.list([_U.list([c,zRow,ns]),_U.list([zCol,i,zCol]),_U.list([s,zRow,c])]);
      return appendBlocks(blocks);
   });
   var rotMatsRI2 = F2(function (theta,n) {
      var makeRI2mat = function (rankI2) {
         var blockR = A2(rotMatR,theta,n - rankI2);
         var blockI2 = idMat(rankI2);
         return blockDiag(_U.list([blockR,blockI2]));
      };
      return A2($List.map,makeRI2mat,_U.range(0,n - 2));
   });
   var rotMats = F2(function (theta,n) {
      var blockMatricesForXEqualTo = function (m) {
         var topLeft = idMat(m);
         var appendBottomRight = function (br) {    return blockDiag(_U.list([topLeft,br]));};
         var lenRest = n - m;
         var bottomRightMats = A2(rotMatsRI2,theta,lenRest);
         return A2($List.map,appendBottomRight,bottomRightMats);
      };
      return $List.concat(A2($List.map,blockMatricesForXEqualTo,_U.range(0,n - 2)));
   });
   var responseDimension = Elm.Native.Port.make(_elm).outboundSignal("responseDimension",
   function (v) {
      return {vertices: Elm.Native.List.make(_elm).toArray(v.vertices).map(function (v) {
                return Elm.Native.List.make(_elm).toArray(v).map(function (v) {    return v;});
             })
             ,edges: Elm.Native.List.make(_elm).toArray(v.edges).map(function (v) {    return [v._0,v._1];})
             ,rotationMatrices: Elm.Native.List.make(_elm).toArray(v.rotationMatrices).map(function (v) {
   ...
   function () {
      var bundler = function (_p20) {
         var _p21 = _p20;
         var _p23 = _p21._1;
         var _p22 = verticesWithEdges(_p23);
         var verts = _p22._0;
         var edgePairs = _p22._1;
         return {vertices: verts,edges: edgePairs,rotationMatrices: A2(rotMats,_p21._0,_p23)};
      };
      return A2($Signal.map,bundler,requestDimension);
   }());
   return _elm.Hypercube.values = {_op: _op,rotMats: rotMats,vertices: vertices,matDim: matDim,verticesWithEdges: verticesWithEdges};
};
